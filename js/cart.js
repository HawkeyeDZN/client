const productsBtn = document.querySelectorAll('.product-item__btn');
...
productsBtn.forEach(el => {
  el.closest('.product-item').setAttribute('data-id', randomId());

  el.addEventListener('click', (e) => {
    let self = e.currentTarget;
    let parent = self.closest('.product-item');
    let id = parent.dataset.id;
    let img = parent.querySelector('.product-item__images').getAttribute('src');
    let title = parent.querySelector('.product-item__title').textContent;
    let priceString = priceWithoutSpaces(parent.querySelector('.product-item__new-price').textContent);
